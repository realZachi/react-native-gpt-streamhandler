 ...
 */
export function ChatHandler({messages, setMessages, apiKey, statusCallback, model = GPTMODELS.GPT3_5_TURBO, url = URL}) {
    ...
    const listener = (event) => {
        if (event.type === "message") {
            handleStreamEvent(event, es, statusCallback, (content) => {
                ...
                setMessages && setMessages((previousMessages) => {
                    return previousMessages.map((m, i) => {
                        if (m === message) {
